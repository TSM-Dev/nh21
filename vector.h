...
struct matrix3x4
{
	matrix3x4()
	{
		memset(this, 0, sizeof(matrix3x4));
	}
...
inline static void VectorRotate(Vector &vecRotation, const Vector &vecIn, const Vector &flAngle)
{
	float flSin, flCos;
	SinCos(Deg2Rad(flAngle.y), &flSin, &flCos);

	vecRotation.x = vecIn.x * flCos - vecIn.y * flSin;
	vecRotation.y = vecIn.x * flSin + vecIn.y * flCos;
	vecRotation.z = vecIn.z;
}
...
inline void Vector::Rotate2D(float yaw)
{
	float sn = sin(Deg2Rad(yaw));
	float cs = cos(Deg2Rad(yaw));

	float nx = this->x * cs - this->y * sn;
	float ny = this->x * sn + this->y * cs;
